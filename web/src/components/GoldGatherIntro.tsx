...
/** Gather → letters lock → DOM title → scatter (title stays) */
const PHASE = {
  GATHER_END: 3.4,
  HOLD_END: 4.0,
  SCATTER_END: 5.6,
...
function edgeSpawn(w: number, h: number, seed: number): Point {
  ...
  const t = (seed * 11.3) % 1
  if (side === 0) return { x: -20 - (seed % 1) * 36, y: t * h * 0.6 }
  if (side === 1) return { x: w + 20 + (seed % 1) * 36, y: t * h * 0.6 }
  if (side === 2) return { x: t * w, y: -20 - (seed % 1) * 28 }
  return { x: t * w, y: h * 0.55 + (seed % 1) * 36 }
}
...
export function GoldGatherIntro({ onComplete }: GoldGatherIntroProps) {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const count = Math.min(1100, Math.max(480, Math.floor((w * h) / 2800)))
      ...
      goldParticles = letters.map((title, i) => {
        ...
        return {
          ...
          seed,
          size: 0.85 + (seed % 1) * 0.55,
          opacity: 0.65 + (seed % 1) * 0.3,
        }
      })
    }
    ...
    const tick = (now: number) => {
      ...
      const gather = smoothstep(0, PHASE.GATHER_END, t)
      const hold = t >= PHASE.GATHER_END && t < PHASE.HOLD_END ? 1 : 0
      const scatter = smoothstep(PHASE.HOLD_END, PHASE.SCATTER_END, t)

      if (t >= PHASE.GATHER_END && !titleDoneRef.current) {
      ...
      const forming = scatter <= 0 && gather < 0.98
      if (forming && gather > 0.25) {
        ctx.shadowColor = 'rgba(255, 200, 80, 0.45)'
        ctx.shadowBlur = 6
      }

      const particleFade = titleVisible ? clamp((t - PHASE.GATHER_END) / 0.35) : 0
      ...
      for (const p of goldParticles) {
        ...
        if (scatter <= 0) {
          alpha *= 0.55 + gather * 0.42
          radius *= 0.9 + gather * 0.4
          if (hold > 0) {
            alpha *= 1 - particleFade * 0.85
          }
        } else {
          alpha *= lerp(0.7, 0.08, scatter) * (1 - particleFade * 0.5)
          radius *= lerp(1.1, 0.55, scatter)
        }
  ...
  return (
    <section className="fixed inset-0 z-50 overflow-hidden bg-[#030508]">
      ...
      <AnimatePresence>
        {titleVisible && (
          <motion.div
            className="pointer-events-none absolute inset-0 z-10 flex flex-col items-center justify-center pt-[2vh] text-center"
      ...
      <div className="fixed bottom-6 right-6 z-[60]">
        <button
          type="button"
          onClick={finish}
          className="liquid-glass px-5 py-2.5 font-serif-cn text-xs tracking-[0.16em] text-[#e8e4d8]/85 transition hover:text-white"
        >
          Skip · 跳过
        </button>
      </div>
