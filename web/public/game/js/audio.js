...
const AudioEngine = {
  ...
  start() {
    ...
    // Organ layer — interstellar-like pad (synthesized, royalty-free)
    const organ = ctx.createOscillator();
    organ.type = 'sine';
    organ.frequency.value = 110;
    const organGain = ctx.createGain();
    organGain.gain.value = 0.012;
    const organFilter = ctx.createBiquadFilter();
    organFilter.type = 'lowpass';
    organFilter.frequency.value = 280;
    organ.connect(organFilter).connect(organGain).connect(ctx.destination);
    organ.start();
    this.nodes.push(organ);

    const organ2 = ctx.createOscillator();
    organ2.type = 'triangle';
    organ2.frequency.value = 164.81;
    const organ2Gain = ctx.createGain();
    organ2Gain.gain.value = 0.008;
    organ2.connect(organ2Gain).connect(ctx.destination);
    organ2.start();
